...
#define EMMC_READY_FOR_DATA	(1<<8)
int mmc_get_mmc_send_status(struct mmc *mmc,char *send_status)
{
	struct mmc_cmd cmd;
	int err;
	uint emmc_status;
	int i,lenth;
	lenth = sizeof(cmd.response)/sizeof(cmd.response[0]);
	do{
		for(i=0;i<lenth;i++)
		{
			cmd.response[i]=0;
		...
		emmc_status = (((uint)cmd.response[3])<<24)|(((uint)cmd.response[2])<<16)|(((uint)cmd.response[1])<<8)|(((uint)cmd.response[0])<<0);
#if 0  //for debug emmc
		//printf("emmc status:");
		//for(i=0;i<lenth;i++)
		//{
		//	printf("cmd.response[%d]:0x%x  ",i,cmd.response[i]);
		//}
		//printf("\n");
#endif
		if(err)
		{
			printf("\nemmc send status err:%d, emmc status:0x%x\n",err,emmc_status);
			break;
		}
	}while(!(emmc_status & EMMC_READY_FOR_DATA) );
...
int mmc_send_ext_csd(struct mmc *mmc, char *ext_csd)
{
	...
	//get the card status 
	mmc_get_mmc_send_status(mmc,NULL);
