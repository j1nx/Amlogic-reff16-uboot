...
#define READ_ONTIME_SIZE 0x200000
int ext2fs_read (char *buf, unsigned len,char c) {
	int status;
    int cur,size;
	if (ext2fs_root == NULL) {
		return (0);
    ...
    for(cur=0,status=0;cur<len;)
    {
        size=(len-cur)>READ_ONTIME_SIZE?READ_ONTIME_SIZE:(len-cur);
	    status += ext2fs_read_file (ext2fs_file, cur, size, &buf[cur]);
	    putc(c);
	    cur+=size;
	    if(status!=cur)
	        return status;
