...
/*if you will define TEST_UBOOT_BOOT_SPEND_TIME to test boot time,you also define TEST_SPL_SPEND_TIME in file(firmware/arm/aml_meson/m3/spl.c  */
#ifdef TEST_UBOOT_BOOT_SPEND_TIME
#define DISPLAY_LOGO_TO_PREPARE_BOOT_KERNEL
#define BOOTM_START_TO_PREPARE_BOOT_KERNEL
/*
uboot boot time calculate:
stage 1: boot spl finished time(us): 0x000b378b = 735115us
stage 2: uboot ucl dec time(us): 43377us
stage 3: board init process time:78368us
stage 4: function init finished time:267670us
stage 5: call loop_main to finish logo display time:176178us
stage 6: board.c display logo finished to prepare boot kernel time:3008097us
stage 7: bootm start to prepare boot kernel time:1577739us
stage 8: from main_loop start to kernel decompress finished time:3184275us

1   from press key to start boot kernel time (normal boot):
    time = (stage 1) + (stage 2) + (stage 3) + (stage 4) + (stage 8) = (4.2s~4.3s) it contain CONFIG_BOOTDELAY=1 time
2   from press key to display logo(A9) time:
    time = (stage 1) + (stage 2) + (stage 3) + (stage 4) + (stage 5) 
3   kernel decompress time: 
    time = stage 7 (about time)
4   from finished display logo(A9) to start boot kernel time:
    time = stage 6 (about time) it contain CONFIG_BOOTDELAY=1 time

if you will want to improve boot process faster that,you can do this:
set environment bootdelay=0 and define CONFIG_BOOTDELAY 0
*/
#endif


#undef CONFIG_COLD_HEART	//for power saving
...
//#define CONFIG_SPL_ENABLE_CACHE 1
