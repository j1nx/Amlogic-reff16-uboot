...
#if defined(CONFIG_EFUSE) && defined(CONFIG_AML_MESION_3)
#include <efuse_m3.h>
#endif


#define FAC_NEW_PCB_VCC5V_LOW_EN
...
int logo_display(void)
{
	...
	int i =0;
	int max_logo_cnt = 20;
	char * p_temp=NULL,*str,temp_buf[128];
	int mode_switch = 0; 	//1:720p   2:576i
	...
	while(1)
	{
		if(power_key_pressed() ==0)
			break;
#if 0
		mode_switch = (mode_switch==2)?1:2;
		if(mode_switch==2)
		{
			run_command ("osd fill 0000ff", 0);
			run_command ("osd open 24 720 480 1", 0);
			run_command ("drawstr 576i 300 300 ff0000", 0);
		...
		else
		{
			...
			run_command ("drawstr 720p 500 350 ff0000", 0);
			run_command ("tv open 720p", 0);
			mdelay(2000);
		}
#endif
		mdelay(polling_time);
		if(power_key_pressed())	//power key still hold on
		{	
			mode_switch = 2;
			break;
		}
    ...
    sprintf(buf,"tv open %s",temp_buf);
    run_command (buf, 0);
#ifdef	CONFIG_UBI_SUPPORT  
//    run_command ("ubi part rs_data", 0);
//    run_command ("ubifsmount rs_data", 0);
    ret =1;
	for(i=0; i<max_logo_cnt; i++)
	{
		sprintf(buf, "ubifsload 82000000 poweron/%04d.bmp", i);
		ret = run_command (buf, 0);
		if(!ret)
			break;
	}
	if(!ret) {
		printf("use dynamic rs_data logo \n");
	}
	else {
    	printf("use default nand logo \n");
...
{
#if 0//#ifdef ENABLE_FONT_RESOURCE    
...
#ifdef FAC_NEW_PCB_VCC5V_LOW_EN
    clrbits_le32(P_AO_GPIO_O_EN_N,(1<<2) | (1<<6));	//This is for new board,GPIOAO_6(VCC5V) LOW-En,High-Disable
    setbits_le32(P_AO_GPIO_O_EN_N,(1<<18) );		//GPIOAO_2 as VDDIO_EN
    clrbits_le32(P_AO_GPIO_O_EN_N,(1<<22));			//GPIOAO_6 as VCC5V_EN
#endif
...
//	printf("get_osd_size-----str is %s",temp_buf);
	sprintf(tvmode, "%s", temp_buf);
...
int need_power_on_recovery(void)
{
	int ret = 0;
	char buf[128];
	#ifdef  CONFIG_UBI_SUPPORT  
    ...
    sprintf(buf, "ubifsload 84100000 upgrade/command");
    ret = run_command (buf, 0);
    if(!ret) {
		printf("/rs_data/upgrade/command is found!!!!: \n");
        return 1;
	}
	else {
		printf("/rs_data/upgrade/command is NOT found!!!!: \n");
        return 0;
	}
#else
		return 0;
...
inline int get_factory_hw_id(void)
{
#ifdef CONFIG_SARADC
	int fac_id_val = 0;
	int hw_ver_val = 0;
	...
	/*CH7 used as factory ID: MICO-9.1%, Genitech-21.2%, SVA-32%*/	
	fac_id_val = get_adc_sample(7);
    printf("fac_id_val==>get_adc_sample(7): 0x%x\n", fac_id_val);
    
    memset(addr,0,sizeof(addr));
    sprintf(addr, "%d", fac_id_val);
    setenv ("facid", addr);

	/*CH6 used as PCB version: 1.0-100%*/
	hw_ver_val = get_adc_sample(6);
	printf("hw_ver_val==>get_adc_sample(6): 0x%x\n", hw_ver_val);
	
    memset(addr,0,sizeof(addr));
    sprintf(addr, "%d", hw_ver_val);
    setenv ("hwid", addr);
...
void check_set_mac(void)
{
	...
	int i;
	char bootargs[384];
	int mac_already_set = 0;
	
	printf("check_set_mac !!!\n");
	if(efuse_getinfo("mac", &info) == 0){
		...
		printf("mac data_len is : %d, i= %d \n", info.data_len, i);
		if(i<info.data_len){
			memset(addr,0,sizeof(addr));
			sprintf(addr,"%02x:%02x:%02x:%02x:%02x:%02x",efuse_data[0],efuse_data[1],efuse_data[2],efuse_data[3],efuse_data[4],efuse_data[5]);

			barg_addr = getenv ("nandargs");
			if(strstr(barg_addr, "mac=")) {
				mac_already_set = 1;
				printf("already setted mac: %s \n", addr);
			}else {
				printf("set efuse mac as: %s \n", addr);
				sprintf(bootargs,"set nandargs '%s mac=%s'",barg_addr,addr);
				run_command (bootargs, 0);
				run_command ("save", 0);
			}
		}else {
			printf("efuse mac data is NOT writen!!! \n");
		}
		
	}else {
		printf("efuse mac is NOT setted !!!\n");
	}
}
#endif
...
//#endif
	...
	if(need_power_on_recovery()) {
		printf("last upgrade file is downloaded to /rs_data,do recovery...\n");
		power_up();
		logo_display();
		run_command ("nand read ${recovery_name} ${loadaddr} 0 ${recovery_size}", 0);
		run_command ("bootm", 0);
