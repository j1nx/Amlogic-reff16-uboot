...
int set_tv_enc_768p(void )
{
    ...
    Wr(ENCP_VIDEO_YFP1_HTIME,       0);
    Wr(ENCP_VIDEO_YFP2_HTIME,       0);

    Wr(ENCP_VIDEO_MAX_PXCNT,        1559);
    Wr(ENCP_VIDEO_HSPULS_BEGIN,     0);
    Wr(ENCP_VIDEO_HSPULS_END,       0);
    Wr(ENCP_VIDEO_HSPULS_SWITCH,    0);
    Wr(ENCP_VIDEO_VSPULS_BEGIN,     0);
    Wr(ENCP_VIDEO_VSPULS_END,       0);
    Wr(ENCP_VIDEO_VSPULS_BLINE,     0);
    Wr(ENCP_VIDEO_VSPULS_ELINE,     0);

    Wr(ENCP_VIDEO_HAVON_BEGIN,      64);
    Wr(ENCP_VIDEO_HAVON_END,        1429);
    Wr(ENCP_VIDEO_VAVON_BLINE,      31);
    Wr(ENCP_VIDEO_VAVON_ELINE,      798);

    Wr(ENCP_VIDEO_HSO_BEGIN,        1);
    Wr(ENCP_VIDEO_HSO_END,          0);
    Wr(ENCP_VIDEO_VSO_BEGIN,        0);
    Wr(ENCP_VIDEO_VSO_END,          0);

    Wr(ENCP_VIDEO_VSO_BLINE,        1);
    Wr(ENCP_VIDEO_VSO_ELINE,        0);
    Wr(ENCP_VIDEO_MAX_LNCNT,        805);
...
static void init_lvds(vmode_t mode)
{
    ...
    if(VMODE_LVDS_768P == mode){
        printf("lvds 768p clock\n");
        ...
        Wr( LVDS_GEN_CNTL, ((Rd(LVDS_GEN_CNTL)&0xfffc) | (1 << 3) | (1<< 0)));
        ...
        //Wr( LVDS_GEN_CNTL, (Rd(LVDS_GEN_CNTL) | (3 << 0)) );    //use dv_continuous as the fifo_wr_en
        // enable the FIFO and use dv_continuous as the fifo_wr_en
    Wr( LVDS_GEN_CNTL, ((Rd(LVDS_GEN_CNTL)&0xfffc) | (1 << 3) | (3<< 0)));
...
static int lvds_set_current_vmode(vmode_t mode)
{
    ...
    if (lvds_init_flag == 0) {
        ...
        WRITE_MPEG_REG(VPP_POSTBLEND_H_SIZE, lvds_info[cur_lvds_index].width);
        /*for (j=0; j<n; j++)
            udelay(1000);*/
        lvds_init_flag = 1;
    }
...
void u_boot_init_lvds()
{
    ...
    lvds_bits = simple_strtoul(getenv ("lvds_bits"), NULL, 16); 
    jeida = simple_strtoul(getenv ("jeida"), NULL, 16); 
    ...
    */
    ...
    if(1 == arg){
        printf("----lvds 768p----\n");
        lvds_set_current_vmode(VMODE_LVDS_768P);
    }else{
        printf("----lvds 1080p----\n");
