...
static int get_charging_status()
{
    //return (READ_CBUS_REG(ASSIST_HW_REV)&(1<<8))? 1:0;//GP_INPUT0
     ...
     struct bq27x00_device_info *di = NULL;
     union power_supply_propval val;
     di = bq27x00_battery_probe(&bq27x00_pdata);
     di->bat.get_property(&(di->bat), POWER_SUPPLY_PROP_STATUS, &val);
     printf("Battery charging status is %d.\n", val.intval);
    
    return  (val.intval == POWER_SUPPLY_STATUS_CHARGING) ? 0 : 1; //return 0 charging else don't charge
}
