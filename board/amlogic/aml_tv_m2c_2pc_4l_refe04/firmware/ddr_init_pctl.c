...
#define ENABLE_POWER_SAVING
    ...
    APB_Wr(PCTL_PHYCR_ADDR, APB_Rd(PCTL_PHYCR_ADDR) & 0xfffffeff );   //disable auto data training when PCTL exit low power state.
      ...
      APB_Wr(PCTL_MCFG_ADDR,((i-4)<<18)|      //tFAW
      						timing_reg->mcfg);//all other setting from $DIR/timming.c
      						                  //DIR search list with priority
      						                  //$(TOPDIR)/board/$(BOARDDIR)/firmware
      						                  //$(SRCTREE)/$(CPUDIR)/$(SOC)/firmware
      						                  //$(SRCTREE)/$(CPUDIR)/common/firmware
      						                  //$(SRCTREE)/$(CPUDIR) /firmware
      						                  //$(SRCTREE)/common/firmware
	  /*
    ...
    APB_Wr(PCTL_ZQCR_ADDR, ( 1 << 31) | (0x7b <<16) );
        ...
        while ( APB_Rd(PCTL_ZQCR_ADDR ) & ( 1<<31) ) {}
         ...
         //APB_Wr(PCTL_ZQCR_ADDR, ( 1 << 24) | 0x11dd); //zqcr move to $DIR/timming.c for board depend
         APB_Wr(PCTL_ZQCR_ADDR, timing_reg->zqcr);	//zqcr setting from $DIR/timming.c
      						                  		//DIR search list with priority
      						                 		//$(TOPDIR)/board/$(BOARDDIR)/firmware
      						                  		//$(SRCTREE)/$(CPUDIR)/$(SOC)/firmware
      						                  		//$(SRCTREE)/$(CPUDIR)/common/firmware
