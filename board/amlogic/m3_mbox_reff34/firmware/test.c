...
//#define POWER_OFF_EE
#define POWER_OFF_VCC
#define RESUME_BY_GPIO_KEY		// gpioao3 used for power key 
#define RTC_AUTO_WAKE_UP		// auto power on by rtc
//#define AMPAK_BOARD

#ifdef POWER_OFF_VCC
void power_off_VCC(int power_on)
{
    // VCC5V GPIOAO6
    // VCCIO GPIOAO2
    ...
    if(power_on)
	{
        clrbits_le32(P_AO_GPIO_O_EN_N, 1<<2); //set GPIO_AO2 to high
        setbits_le32(P_AO_GPIO_O_EN_N, 1<<18); //set GPIO_AO2 to high  VDDIO_EN
#ifdef AMPAK_BOARD
        clrbits_le32(P_AO_GPIO_O_EN_N, 1<<6); //set GPIO_AO6 to high
        setbits_le32(P_AO_GPIO_O_EN_N, 1<<22); //set GPIO_AO6 to high  VDD5V_EN
#else
        clrbits_le32(P_AO_GPIO_O_EN_N, (1<<22)|(1<<6)); //set GPIO_AO6 to low  VCC5V_EN
#endif
	}
	else
	{
        clrbits_le32(P_AO_GPIO_O_EN_N, (1<<18)|(1<<2)); //set GPIO_AO2 to low  VDDIO_EN
        clrbits_le32(P_AO_GPIO_O_EN_N, 1<<6); //set GPIO_AO6 to high
#ifdef AMPAK_BOARD
        clrbits_le32(P_AO_GPIO_O_EN_N, 1<<22); //set GPIO_AO6 to low
#else
        setbits_le32(P_AO_GPIO_O_EN_N, 1<<22); //set GPIO_AO6 to high
#endif
	}
}
#endif
void enter_power_down()
{
    ...
    while(1){
        ...
        if(!power_key) break;
#endif /*RESUME_BY_GPIO_KEY*/
        }
    //f_serial_puts("Detect Power-On Signal\n");
#endif /*AMPAK_BOARD*/
...
//	setbits_le32(P_A9_CFG2, 7<<16);
  ...
  rtc_ctrl = readl(0xC810074c);
    ...
    while(1)
    {
    	//detect remote key
		  power_key=readl(P_AO_IR_DEC_FRAME);
		  if(power_key==0xfc03ef10)  //the reference remote power key code
        break;
