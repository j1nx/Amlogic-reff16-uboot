...
{
#ifdef CONFIG_AML_LVDS_1366x768
	{/* VMODE_1080P */
		   .name			  = "lvds768p",
...
#endif
#ifdef CONFIG_AML_LVDS_1920x1080
    {/* VMODE_1080P */
        .name              = "lvds1080p50hz",
...
static int lvds_set_current_vmode(vmode_t mode)
{
    ...
    long m = 10, n = 10;
    for(i=0;i<count;i++){
        if(lvds_info[i].mode==(mode&VMODE_MODE_BIT_MASK)){
            cur_lvds_index = i;
            break;
        }
    }
    if(i == count)
        return -EINVAL;
    if(VMODE_LOGO_BIT_MASK& mode)
		return 0;
	printk("lvds_set_current_vmode = %d\n",mode);
    /* to do */
    if (lvds_init_flag == 0) {
        /*for (j=0; j<m; j++)
            udelay(1000);*/
...
void u_boot_init_lvds()
{
    ...
    int m = 10, n = 34;
		...
		if(reverse == 1){
			...
			unsigned long jeida_reverse = simple_strtoul(getenv ("jeida"), NULL, 16);
		if(jeida_reverse == 1){
			jeida = 1;
			}
		printk("vout lvds resume port_reverse=%d\n",port_reverse);
