...
#define I2C_AXP20_ADDR   (0x68 >> 1)
void i2c_axp20_write(unsigned char reg, unsigned char val)
{
...
unsigned char i2c_axp20_read(unsigned char reg)
{
    unsigned char val = 0;
    struct i2c_msg msgs[] = {
        {
...
void close_limit(int flag)
{
    static unsigned char reg3;
    
    if(flag) {
        reg3 = i2c_axp20_read(0x81);
        i2c_axp20_write(0x81,0);
    }
    else {
        i2c_axp20_write(0x81,reg3);
    }
    
}
void vddio_off() // ext-en: bit0 
{
	unsigned char reg3;

	
	reg3 = i2c_axp20_read(0x12);
	reg3 &= ~(1<<0);	
	i2c_axp20_write(0x12,reg3);
//	serial_puts("\n ext-en_off!\n");
}
...
//DDR vcc 1520<-->1420
//1.2V AO 1225<-->1050
void vcc_down() // ext-en: bit0 
{
	unsigned char reg3;  
	
	reg3 = i2c_axp20_read(0x23);
	reg3 &= 0xc0;
	reg3 |= 0x1c;
	i2c_axp20_write(0x23,reg3);

	reg3 = i2c_axp20_read(0x27);
	reg3 &= 0x80;
	reg3 |= 0x8;
	i2c_axp20_write(0x27,reg3);
}
//DDR vcc 1520<-->1420
//1.2V AO 1220<-->1050
//µÁ—πª÷∏¥
...
//close AVDD33  ldo4:bit3
void reg5_off()
{
	unsigned char data;
	
	data = i2c_axp20_read(0x12);
	data &= ~(1<<3);		
	i2c_axp20_write(0x12,data);
//	serial_puts("\n ldo4_off!\n");
}

//open AVDD33  ldo4:bit3
void reg5_on()
{
	unsigned char reg3;
...
//91H  bit4-7  1.8-3.3 
	...
	reg3 = i2c_axp20_read(0x90);	
	reg3 &= 0xf8;
	reg3 |= (3);	
	i2c_axp20_write(0x90,reg3);

	reg3 = i2c_axp20_read(0x91);	
	reg3 &= 0xf;
	reg3 |= (0xf0);	

	i2c_axp20_write(0x91,reg3);
}
...
//	delay_ms(20);
	//---------------------------------
	//config	  
 	v=20; //20 = 400HZ (32000/speed) >>2) speed:400K
