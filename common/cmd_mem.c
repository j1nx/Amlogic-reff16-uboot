...
int do_mem_mtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	ulong	val;
	ulong	readback, readback_nocache;
	ulong	errs = 0;
	int iterations = 1;
	int iteration_limit;
	...
	for (;;) {
		...
		for (addr=start,val=pattern; addr<end; addr++) {
			...
			if (readback != val) {
			    if (addr>=0x80000000){
    			    readback_nocache = *(uint*)((uint)addr+0x20000000);
    			    if ((readback_nocache!=readback)&&(readback_nocache==val))
    				printf ("\nData Cache error @ 0x%08X: "
    					"found %08lX in dcache, %08lX in DDR, expected %08lX\n",
    					(uint)addr, readback, readback_nocache, val);
                    else
    				printf ("\nMem error @ 0x%08X: "
    					"found %08lX in dcache, %08lX in DDR, expected %08lX\n",
    					(uint)addr, readback, readback_nocache, val);
