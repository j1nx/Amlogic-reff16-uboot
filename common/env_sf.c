...
u8 mac_buf[20];
void read_mac_address_from_spi(void)
{
    u8 buff[6];
    puts("Read MAC from SPI ...\n");
    memset(buff, 0, 6);
    memset(mac_buf, 0, 20);
    spi_flash_read(env_flash, CONFIG_MAC_OFFSET, 6, buff);
    sprintf(mac_buf, "%02x:%02x:%02x:%02x:%02x:%02x", buff[0], buff[1], buff[2], buff[3], buff[4], buff[5]);
}
#endif
...
err_crc:
	...
	set_default_env();
        ...
        read_mac_address_from_spi();
#endif
#ifdef WRITE_DEFAULT_ENV_IN_SPI
        saveenv();
#endif
}
