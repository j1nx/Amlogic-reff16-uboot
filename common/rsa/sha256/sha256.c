...
#define SHA256_BUFF_DUMP

#ifdef SHA256_BUFF_DUMP
	printf("DATA before sha256 : length=%d\n",len);
	for(i = 0;i< len;++i)
	printf( "%02X",msg[i]);
	printf("\n");
#endif
...
#ifdef SHA256_BUFF_DUMP
	unsigned char *pBuff = (unsigned char *)dest;
	printf("DATA after sha256 :\n");
	for(i = 0;i< sizeof(sha256_hash_t);++i)
	printf( "%02X",pBuff[i]);
	printf("\n");
#endif	
