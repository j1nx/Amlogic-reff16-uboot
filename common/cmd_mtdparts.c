...
static int part_validate(struct mtdids *id, struct part_info *part)
{
	...
	if (part->offset + part->size > id->size) {
		printf("part->offset:0x%x; part->size:0x%x; id->size:0x%x\n",
			part->offset, part->size, id->size);
...
static int part_parse(const char *const partdef, const char **ret, struct part_info **retpart)
{
	...
	*ret = NULL;
	...
	part->size = 0x11111111;
	//printf("1111part:0x%x; &part->size:0x%x\n", part, &part->size);
	//printf("ELVIS-aa-part->size:%x; xxsize:%llx\n", part->size, *((unsigned long *)(&part->size)));
	part->size = size;
	//printf("ELVIS-cc-size:%llx\n", size);
	part->offset = offset;
	part->mask_flags = mask_flags;
	part->name = (char *)(part + 1);
	//printf("ELVIS-bb-part->size:%llx; yysize:%llx\n", part->size, *((unsigned long *)(&part->size)));
	if (name) {
