 ...
 nf_read(unsigned target, unsigned size)
{
    ...
    clrbits_le32(P_PERIPHS_PIN_MUX_6,1<<28);
    clrbits_le32(P_PERIPHS_PIN_MUX_5,0x1f<<1);
    
    switch(POR_GET_INTL_CFG(por_cfg))
    {
        case POR_INTL_NAND:
        case POR_INTL_NAND_SP:
            setbits_le32(P_PERIPHS_PIN_MUX_4,(0x1ff<<11));
            break;
        case POR_INTL_NAND_RB:
        case POR_INTL_NAND_SP_RB:
            setbits_le32(P_PERIPHS_PIN_MUX_4,(0x1<<11) | (0xff<<14));
            break;
    }
