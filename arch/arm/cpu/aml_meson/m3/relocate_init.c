...
static void sdio_read(unsigned target,unsigned src_offset,unsigned size,unsigned por_sel);
...
//	asm("wfi");
	...
	}else if(boot_id)
	{
		...
		sdio_read(__TEXT_BASE,READ_SIZE,size,por_cfg&POR_SDIO_CFG_MASK);
	}
	else{
...
re_nand:
			...
			case  POR_INTL_SDIO_B1:
				sdio_read(__TEXT_BASE,READ_SIZE,size,0);
				break;
			default:
...
static void sdio_read(unsigned target,unsigned src_offset,unsigned size,unsigned por_sel)
{
   ...
   size=(size+511)&(~(511));
   for(addr=src_offset/*addr=READ_SIZE*/,read_size=0;read_size<size;addr+=CONFIG_SDIO_BUFFER_SIZE)
   {
