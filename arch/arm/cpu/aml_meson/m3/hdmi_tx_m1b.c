...
void hdmi_hw_set_powermode( int power_mode, int vic)
{
    ...
    switch(power_mode){
        case 1:
            ...
            hdmi_wr_reg(TX_CORE_CALIB_VALUE, 0xf);  //0xf7    temporary setting, need delete later
#ifdef MORE_LOW_P
...
//////////////////////////////reset    
    ...
    tmp_add_data = 0xc0;        // using HDMI output mode
...
static void hdmi_audio_init(unsigned char spdif_flag)
{
    ...
    hdmi_wr_reg(TX_AUDIO_FIFO, 0x49); // 0x49
...
static void hdmitx_dump_tvenc_reg(int cur_VIC, int printk_flag) 
{
    ...
    for(i=0;hdmi_tvenc_configs[i].vic!=HDMI_Unkown;i++){
        if(cur_VIC==hdmi_tvenc_configs[i].vic){
            ...
            for(j=0;reg_set[j].reg;j++){
                hdmi_print(printk_flag, "[%08x]=%08x=%08x\n",reg_set[j].reg, reg_set[j].val, Rd(reg_set[j].reg));
           }
...
//            Wr(HHI_VID_CLK_DIV, 3);      //0x1059
        ...
        case HDMI_1080i60:
        case HDMI_1080i50:
            Wr(HHI_VID_PLL_CNTL, (1<<10)|(31<<0));    //74.2MHz=24MHz*371/12/10
//            Wr(HHI_VID_CLK_DIV, 0);      //0x1059
        ...
        case HDMI_1080p60:
        case HDMI_1080p50:
            Wr(HHI_VID_PLL_CNTL, (1<<10)|(62<<0));    //148.4MHz=24MHz*371/6/10
...
static int hdmitx_m3_set_dispmode(Hdmi_tx_video_para_t *param)
{
    ...
    printk("set mode VIC %d (cd%d,cs%d,pm%d,vd%d,%x) \n",
                param->VIC, 
                color_depth_f, 
                color_space_f,
                power_mode,
