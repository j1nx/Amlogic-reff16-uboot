...
static mlvds_dev_t *pDev = NULL;

static void _mlvds_init(lcdConfig_t *pConf) ;
...
static void _init_vout(mlvds_dev_t *pDev)
{
...
int mlvds_probe(void)
{
    pDev = (mlvds_dev_t *)malloc(sizeof(mlvds_dev_t));
    if (!pDev) {
    ...
    _mlvds_init(&pDev->conf);
    return 0;
}

int mlvds_remove(void)
{
    free(pDev);
    return 0;
}
