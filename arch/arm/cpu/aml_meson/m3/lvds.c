...
static lvds_dev_t *pDev = NULL;

static void _lvds_init(lcdConfig_t *pConf) ;
...
static void _init_vout(lvds_dev_t *pDev)
{
...
int lvds_probe(void)
{
    pDev = (lvds_dev_t *)malloc(sizeof(lvds_dev_t));
    if (!pDev) {
        printf("[lvds]: Not enough memory.\n");
        return -1;
    }

    pDev->conf = lcd_config;

    _lvds_init(&pDev->conf);
    return 0;
}

int lvds_remove(void)
{
    free(pDev);
    return 0;
}
