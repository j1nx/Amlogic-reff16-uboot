/*
 * (C) Copyright C 2011 by Amlogic, Inc.
 *
 *
...
static char * str_1st_str[]={"Disable","SDIO A","SDIO B","SDIO C",
        "SPI Flash","SPI Flash","NAND Flash RB Mode","NAND Flash Without RB"
    };
...
{
    int i,j;
    unsigned por=C_ROM_BOOT_DEBUG->por_cfg;
    ...
    for(i=0;i<2;i++)
    {
        ...
        printf("Init result %x\n",C_ROM_BOOT_DEBUG->init[i]);
        for(j=0;j<4;j++)
        {
            printf("area %d load result %x\n",j,C_ROM_BOOT_DEBUG->load[i][j]);
            printf("area %d check result %x\n",j,C_ROM_BOOT_DEBUG->dchk[i][j]);
        }
    }
